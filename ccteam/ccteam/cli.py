...
Manual mode: subcommands like ``ccteam claim foo.py``,
``ccteam status``, ``ccteam peers`` — useful for debugging
...
def format_block_message(path: str, data: dict) -> str:
    ...
    return (
        f"ccteam: {path} is held by {holder_desc}. "
        f"Their edit is still in progress after a {HOOK_CLAIM_TIMEOUT_MS // 1000}s grace.\n"
        "Surface this to the user. Options:\n"
        f"  - Wait longer: call mcp__ccteam__claim with paths=['{path}'] and timeout_ms=30000\n"
        "  - Check peers: call mcp__ccteam__peers to see who is connected\n"
        f"  - Preempt: call mcp__ccteam__force_claim with paths=['{path}'] and a reason\n"
...
async def cmd_claim(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"No ccteam server running for {root}", file=sys.stderr)
        return 1
...
async def cmd_session_start(args: argparse.Namespace) -> int:
    """SessionStart hook: surface a one-line notice to the user when NATS
    is unreachable in a ccteam-aware project. Stays silent in projects that
    don't use ccteam, and silent when NATS is healthy.
    """
    # Only speak up in projects that have actually bootstrapped ccteam.
    pr = project_root_mod.find()
    if pr.marker != ".ccteam":
        return 0
    ...
    # Short-timeout liveness probe — session start must not stall.
    try:
        with contextlib.redirect_stderr(io.StringIO()):
            nc = await asyncio.wait_for(
                nats.connect(
                    cfg.nats_url,
                    name="ccteam-session-check",
                    connect_timeout=1,
        ...
        await nc.close()
        return 0  # NATS healthy — no notice
    except Exception:
        pass

    notice = (
        f"ccteam: NATS unreachable at {cfg.nats_url}. Running in STANDALONE mode — "
        "filesystem sync across non-shared project directories is unavailable. "
        "Start `nats-server -js` (or set CCTEAM_NATS_URL) to enable cross-instance coordination."
    )
    print(json.dumps({
        "hookSpecificOutput": {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="ccteam")
    ...
    sub.add_parser(
        "session-start",
        help="SessionStart hook: warn user if NATS is unreachable in a ccteam project",
    )
