...
pattern in ``{root}/.ccteam/ignore`` or the built-in `DEFAULT_IGNORE`.
...
def load_ignore(root: Path) -> tuple[str, ...]:
    """Combine built-in defaults with any patterns from `.ccteam/ignore`."""
    patterns = list(DEFAULT_IGNORE)
    ignore_file = root / ".ccteam" / "ignore"
