...
MCP_INSTRUCTIONS = """\
ccteam coordinates file edits across multiple Claude Code instances \
...
def build_mcp(node: node_mod.Node) -> FastMCP:
    mcp = FastMCP(name="ccteam", instructions=MCP_INSTRUCTIONS)
    ...
    async def checkpoint(message: str = "") -> dict:
        ...
        Overwrites `.ccteam/checkpoint.json` and `manifest.json`,
        ...
        return {
            ...
            "reminder": "Commit .ccteam/checkpoint.json and .ccteam/manifest.json to share this checkpoint with peers.",
        }
...
async def run(
    ...
    try:
        ...
        print(f"ccteam startup: {exc}", file=sys.stderr)
        sys.exit(2)
    except Exception as exc:
        print(f"ccteam startup failed: {exc}", file=sys.stderr)
        raise

    ipc: local_ipc.Server | None = None
    if node.standalone:
        # Single stderr line at startup (goes to Claude Code's MCP log, not
        # the user-visible conversation). Don't start the IPC server: with
        # no socket, the PreToolUse hook short-circuits to exit 0, so
        # Edit/Write are never blocked or annotated.
        print(
            f"ccteam: NATS unreachable at {node.cfg.nats_url} — running in STANDALONE mode. "
            "Filesystem sync across non-shared project dirs is unavailable.",
            file=sys.stderr,
        )
    else:
        if node.p2p:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ccteam MCP server")
