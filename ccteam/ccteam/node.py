...
class Node:
    ...
    async def start(
        ...
        try:
            ...
            # NATS unreachable — degrade to standalone. Filesystem sync and
            # cross-instance coordination are disabled; MCP server still starts.
            log.warning(
                "NATS unreachable at %s (%s) — ccteam running in STANDALONE mode. "
                "Filesystem sync across non-shared project dirs is unavailable.",
                self.cfg.nats_url, exc,
            )
        ...
        if local is None:
            ...
            raise StartupDivergence(
                reason=(
                    ...
                    "`.ccteam/checkpoint.json` exists and the local "
    ...
    def verify(self) -> dict:
        ...
        for path, local_hash in live.entries.items():
            ...
            if exp is None:
                # File is in live but ccteam doesn't know about it.
