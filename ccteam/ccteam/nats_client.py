"""Thin wrapper around the NATS client for ccteam.
...
def locks_bucket_name(cluster_id: str) -> str:
    return f"ccteam-{cluster_id}-locks"


def events_stream_name(cluster_id: str) -> str:
    return f"ccteam-{cluster_id}-events"


def objects_bucket_name(cluster_id: str) -> str:
    return f"ccteam-{cluster_id}-files"
    ...
    try:
        ...
        await js.add_stream(
            config=StreamConfig(
                ...
                description="ccteam overlay events",
            )
        )
    ...
    try:
        ...
        return await js.create_object_store(
            ...
            config=ObjectStoreConfig(
                bucket=bucket,
                storage=StorageType.FILE,
                description="ccteam file contents",
            ),
        )
...
async def bootstrap(nats_url: str, cluster_id: str, node_id: str) -> NatsResources:
    ...
    """
    nc = await connect(nats_url, name=f"ccteam-{node_id}")
    js = nc.jetstream()
