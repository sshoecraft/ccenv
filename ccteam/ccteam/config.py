"""Configuration loading for ccteam.

Precedence, highest first:

1. Values passed explicitly to `load()`.
2. Environment variables (``CCTEAM_*``).
3. Project-level ``{project_root}/.ccteam/config.json``.
4. User-level ``~/.ccteam/config.json``.
...
def load(project_root: Path | None = None, **overrides: Any) -> Config:
    ...
    user_file = Path.home() / ".ccteam" / "config.json"
    for key, value in _load_file(user_file).items():
        if key in known:
            data[key] = _coerce(key, value)

    if project_root is not None:
        project_file = project_root / ".ccteam" / "config.json"
