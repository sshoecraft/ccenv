...
| Kind | Name |
|---|---|
| KV bucket | `ccteam-<cluster_id>-locks` |
| JetStream stream | `ccteam-<cluster_id>-events` |
| Object store | `ccteam-<cluster_id>-files` |
...
Both live under `.ccteam/` at the project root.
...
ccteam does not touch `.git/` or know about branches.

## Local IPC (hook ↔ server)

Socket path: `~/.ccteam/runtime/<sha256(project_root)[:16]>.sock`.
