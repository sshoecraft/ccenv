...
def test_env_overrides(tmp_path: Path) -> None:
    with mock.patch.dict(
        os.environ,
        {
            "CCTEAM_NATS_URL": "nats://other:4222",
            "CCTEAM_DISCOVERY_PORT": "9999",
            "CCTEAM_SHARED": "true",
        },
        clear=True,
    ):
...
def test_explicit_overrides_win(tmp_path: Path) -> None:
    with mock.patch.dict(
        os.environ, {"CCTEAM_NATS_URL": "nats://env:4222"}, clear=True,
    ):
