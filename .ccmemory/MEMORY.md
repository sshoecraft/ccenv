...
- [ccloop-interactive-relay-orphans-child-processes](ccloop-interactive-relay-orphans-child-processes.md) — TWO orphan mechanisms, both fixed: (1) ccloop's own abnormal death -> PDEATHSIG (v0.6.1); (2) relay SIGTERM hitting a CCLOOP_CLAUDE_BIN shell wrapper…
...
- [mcp-alwaysload-blocks-startup](mcp-alwaysload-blocks-startup.md) — CORRECTED: alwaysLoad is a 5s per-TIER timeout that PROCEEDS degraded, not a barrier. SessionStart hooks run before init, so no hook can gate on MCP…
